...
class SiteController extends Controller {
    ...
    public function actionCreatedGID() {
        ...
        if (isset($_POST['selectedIds'])) {
            ...
            Yii::import('application.modules.file_toArray');
            //  $file_toArray = new file_toArray();
            //  $file_toArray->update_csv_correctedGID($fid, $mid, $fid);
        }
       ...
       //  echo "before:".count($final);
        ...
        if (count($final)) {
            ...
            if (count($pages) != 0) {
              ...
              //  echo "final:".count($final);
               ...
               echo ("final_arr:");
                      ...
                      foreach ($final[0] as $i => $r) : list($id,$nval,$term,$GID, $methodID, $method, $locID, $location) = $r;
                          if($id==$fem[0] || $id==$mal[0] || (strpos($id,$fem[0]) || strpos($id,$mal[0])) ){
                              
                               $arr1[$i]= array('id'=>$id,'nval'=>$nval,'term'=>$term,'GID'=>$GID,'methodID'=>$methodID,'method'=>$method,'locID'=>$locID,'location'=>$location);
                                //echo "<br>".$i;
                               // print_r(array_merge($arr1[$i]));
                                $final_arr = array_merge($final_arr,$arr1[$i]);
                              // $final_arr = array_merge($final_arr,$arr1[$i]);
                           }
                      endforeach;
           ...
           $dataProvider = new CArrayDataProvider($final_arr, array(                 
                    'keyField'=> 'id',
                    'pagination' => array(
                         'pageSize' => 1,
                    ),
         ...
         if(Yii::app()->request->isAjaxRequest) $this->renderPartial('createdGID', array('dataProvider' => $dataProvider,'GdataProvider'=>$GdataProvider),false,true);
         else $this->render('createdGID', array('dataProvider' => $dataProvider,'GdataProvider'=>$GdataProvider));
    ...
    /*public function actionShowChooseGID(){
        $id = $_POST['id'];
        $model = ChooseGID::model()->findByPk($fixedLineId);
    }*/
/**
