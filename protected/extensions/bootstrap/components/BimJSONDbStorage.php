...
class BimJSONDbStorage extends CComponent
{
	/**
	...
	{
		$this->filename = $this->path . (isset(Yii::app()->session['user_id']) ? md5(Yii::app()->session['id']) : md5(date('YmdHis'))) . '-' . $file;
	}
...
//		if (file_put_contents($this->getFile(), $encodedData))
//        {
            if (!empty(Yii::app()->session['user_id']))
            {
                $browserSession = BrowserSession::model()->findByAttributes(array('user_id' => Yii::app()->session['user_id']));
                
                if (empty($browserSession))
                    $browserSession = new BrowserSession;
                
                $browserSession->user_id = Yii::app()->session['user_id'];
                $browserSession->contents = $encodedData;
//                $browserSession->file = $this->getFile();

                if ($browserSession->save())
                    return true;
                else
                    throw new Exception(strtr('Unable to save {FILE} to database. Data will be saved on local only!', array('{FILE}' => $this->getFile())));
            }
//        }
//        else
...
//		if (file_exists($this->getFile()))
        if (!empty($browserSession->contents))
		{
//			$json = file_get_contents($this->getFile());;
//            
//            if (strlen($json) == 0)
//				return;
            $json = $browserSession->contents;
