<html>
<?php
//include_once($_SERVER['DOCUMENT_ROOT'] . "/PedigreeImport/header.php");
                                        ...
                                        for ($i = 0; $i < count($pages); $i++) {
                                            foreach ($pages[$i] as $r) : list($id, $nval, $term, $GID, $methodID, $method, $locID, $location) = $r;
												...
												if($id==$femIdArr[0]){ //female 
                                                    echo "<td bgcolor='#FFE4E1'><img src='images/imgFemale.jpg' height='1' width='15'>"." ". $term . "</td>";
											    }else if($id==$maleIdArr[0]) //male
											    {
													  echo "<td bgcolor='#E6E6FA'><img src='images/imgMale.jpg' height='1' width='15'>"." ". $term . "</td>";
                                                ...
                                                if ($GID === "CHOOSE GID") {
                                                    if($id==$femIdArr[0]){ //female
														echo "<td bgcolor='#FFE4E1'><a data-toggle='modal' href='?term=" . $term . "&id=" . $id . "&pedigree=" . $nval . "&mid=" . $mid . "&fid=" . $fid . "&female=" . $female . "&male=" . $male . "#form-content' >Choose GID</a></td>";
													}else if($id==$maleIdArr[0]){ //male
														echo "<td bgcolor='#E6E6FA'><a data-toggle='modal' href='?term=" . $term . "&id=" . $id . "&pedigree=" . $nval . "&mid=" . $mid . "&fid=" . $fid . "&female=" . $female . "&male=" . $male . "#form-content' >Choose GID</a></td>";
													}
													else if(strpos($id,'/')){ //parent
														echo "<td bgcolor='#90EE90'><a data-toggle='modal' href='?term=" . $term . "&id=" . $id . "&pedigree=" . $nval . "&mid=" . $mid . "&fid=" . $fid . "&female=" . $female . "&male=" . $male . "#form-content' >Choose GID</a></td>";
													}
														//echo "<td><a  href='/PedigreeImport/assignGID.php?term=" . $term . "&id=" . $id . "&pedigree=" . $nval . "&mid=" . $mid . "&fid=" . $fid . "&female=" . $female . "&male=" . $male . "#form-content' >Choose GID</a></td>";
														$m_term = $term;
														$m_id = $id;
														$m_pedigree = $nval;
														$m_nval = $nval;
...
<!--
    ...
    <script type="text/javascript">
                function show(var1, var2, var3, var4) {
                    $.pnotify(
                            {
                                text: var4 + " rows selected \n    " + var1 + " remaining rows to be processed",
                                type: "info",
                                hide: false,
                                shadow: false,
                                opacity: .8
                    ...
                    /* $.pnotify(
                     {
                     text: var3 + " row(s) selected with unstandardized germplasm name(s)",
                     type: "warning",
                     hide: false,
                     shadow: false,
                     opacity: .8
